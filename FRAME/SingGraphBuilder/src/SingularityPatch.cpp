...
{}
/*----------------------------------------------------------------------------*/
void SingularityPatch::addPoint(SingularityPoint* APoint)
{
  if(APoint==0)
    throw GMDSException("SingularityPatch - Try to add a null point");
  m_points.push_back(APoint);
  m_valid_flag = false;
}
/*----------------------------------------------------------------------------*/
void SingularityPatch::addLine(SingularityLine* ALine)
{ 
  if(ALine==0)
    throw GMDSException("SingularityPatch - Try to add a null line");
  m_lines.push_back(ALine);
  m_valid_flag = false;
}
/*----------------------------------------------------------------------------*/
bool SingularityPatch::getPoints(std::vector<SingularityPoint*>& APoints)
{
  if(!m_valid_flag)
    m_valid_flag = checkValidityAnReorder();
...
/*----------------------------------------------------------------------------*/
bool SingularityPatch::getLines(std::vector<SingularityLine*>& ALines)
{  
  if(!m_valid_flag)
    m_valid_flag = checkValidityAnReorder();
...
/*----------------------------------------------------------------------------*/
bool SingularityPatch::checkValidityAnReorder()
{  
  //==========================================================================
  ...
  for(unsigned int i=0; i < m_points.size(); i++){
    ...
    if( nb_connected<1 || nb_connected>2)
      return false;
  ...
  for(unsigned int i=0;i<adj_lines.size();i++)
    for(unsigned int j=0;j<m_lines.size();j++)
      if(adj_lines[i]==m_lines[j])
	candidate_lines.push_back(adj_lines[i]);
