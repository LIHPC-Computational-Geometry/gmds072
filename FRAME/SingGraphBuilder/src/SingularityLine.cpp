...
/*----------------------------------------------------------------------------*/
SingularityLine::SingularityLine(const  ESingularityGeomLineType AType)
:m_type(AType)
{}
/*----------------------------------------------------------------------------*/
...
{}
/*----------------------------------------------------------------------------*/
void SingularityLine::
addSingularityPoint(SingularityPoint* APnt)
{
    if (m_points.size() >= 2)
        throw gmds::GMDSException("A singularity line can only have two end points");
    
    m_points.push_back(APnt);
    
}
/*----------------------------------------------------------------------------*/
void SingularityLine::
addDiscretizationPoint(const gmds::math::Point& APnt){
    m_discretization_points.push_back(APnt);
}
/*----------------------------------------------------------------------------*/
void SingularityLine::
setDiscretizationPoints(const std::vector<gmds::math::Point >& APoints)  {
    m_discretization_points = APoints;
}
/*----------------------------------------------------------------------------*/
std::vector<gmds::math::Point >& SingularityLine::getDiscretizationPoints(){
    return m_discretization_points;
}
/*----------------------------------------------------------------------------*/
std::vector<SingularityPoint*>& SingularityLine::getEndPoints(){
    return m_points;
}
/*----------------------------------------------------------------------------*/
bool SingularityLine::healOrientation()
{
    //We need two end points
    if(m_points.size()!=2)
        return false;
    
    gmds::math::Point first_end  = m_points[0]->getLocation();
    gmds::math::Point second_end = m_points[1]->getLocation();
    ...
    if(!gmds::math::near(d1,0.0) && !gmds::math::near(d2,0.0) &&
       !gmds::math::near(d3,0.0) && !gmds::math::near(d4,0.0) )
        return false;
...
/*----------------------------------------------------------------------------*/
bool CurveSingularityLine::healOrientation()
{
...
/*----------------------------------------------------------------------------*/
void SurfaceSingularityLine::
smooth(const int ANbStep)
{
    std::cout << "Smoothing is not implemented for surface singularity lines!!"<<std::endl;
}
/*----------------------------------------------------------------------------*/
void SurfaceSingularityLine::addTraversedFace(const gmds::TCellID AID)
{
    m_traversed_faces_id.push_back(AID);
}
/*----------------------------------------------------------------------------*/
std::vector<gmds::TCellID> SurfaceSingularityLine::
getTraversedFaces() const
{
    return m_traversed_faces_id;
}
/*----------------------------------------------------------------------------*/
bool SurfaceSingularityLine::
isTraversed(const gmds::TCellID AFaceID) const
{
    for(unsigned int i=0;i<m_traversed_faces_id.size();i++) {
        if(AFaceID == m_traversed_faces_id[i])
            return true;
    }
...
/*----------------------------------------------------------------------------*/
gmds::math::Vector SingularityLine::getTangent(const double AParam) const
{
    if(AParam<0 || AParam >1)
        throw gmds::GMDSException("The line parameter must be in [0,1]");
    
    if(m_discretization_points.empty())
        throw gmds::GMDSException("Line discretization must be known to compute tangent");
    
    gmds::math::Vector t;
    if(AParam==0){
        gmds::math::Vector v1(m_discretization_points[0], m_discretization_points[1]);
        gmds::math::Vector v2(m_discretization_points[1], m_discretization_points[2]);
        t=0.5*(v1+v2);
    }
    else if (AParam==1){
        gmds::math::Vector v1(m_discretization_points[m_discretization_points.size()-2],
                              m_discretization_points[m_discretization_points.size()-1]);
        gmds::math::Vector v2(m_discretization_points[m_discretization_points.size()-3],
                              m_discretization_points[m_discretization_points.size()-2]);
        t=0.5*(v1+v2);
    }
    else {
        ...
        while (c<l){
            ...
            if(c+dij>=l){
                ...
                if(i==0){
                    ...
                    gmds::math::Vector vjk(pj,pi);
                    double r = 1.0/3.0;
                    gmds::math::Vector v =r*(vhi+vij+vjk);
                    v.normalize();
                    return v;
                }
            }
...
/*----------------------------------------------------------------------------*/
gmds::math::Point SingularityLine::getPoint(const double AParam) const
{
    if(AParam<0 || AParam >1)
        throw gmds::GMDSException("The line parameter must be in [0,1]");
    ...
    for (int k = 0; k < ANbStep; k++) {
        for (unsigned int i = 0; i < m_discretization_points.size(); i++)	{
            if (i == 0 || i == m_discretization_points.size() - 1)   {
                ...
                gmds::math::Point(0.309*p0.X() + 0.382*p1.X() + 0.242*p2.X() + 0.061*p3.X() + 0.006*p4.X(),
                ...
                new_discretization[i] = gmds::math::Point(
                                                          0.309*p0.X() + 0.382*p1.X() + 0.242*p2.X() + 0.061*p3.X() + 0.006*p4.X(),
                                                          0.309*p0.Y() + 0.382*p1.Y() + 0.242*p2.Y() + 0.061*p3.Y() + 0.006*p4.Y(),
                                                          0.309*p0.Z() + 0.382*p1.Z() + 0.242*p2.Z() + 0.061*p3.Z() + 0.006*p4.Z()
                                                          );
            }
                ...
                new_discretization[i] = gmds::math::Point(
                                                          0.067*p0.X() + 0.242*p1.X() + 0.382*p2.X() + 0.242*p3.X() + 0.061*p4.X() + 0.006 *p5.X(),
                ...
                new_discretization[i] = gmds::math::Point(
                                                          0.006*p0.X() + 0.061*p1.X() + 0.242*p2.X() + 0.382*p3.X() + 0.242*p4.X() + 0.061*p5.X() + 0.006 *p6.X(),
        ...
        m_discretization_points.insert(m_discretization_points.end(),
                                       new_discretization.begin(), new_discretization.end());
  }
}
/*----------------------------------------------------------------------------*/
SingularityPoint* SingularityLine::removeSingularityPoint()
{
  SingularityPoint* p = m_points.back();
  m_points.pop_back();
  return p;
}
/*----------------------------------------------------------------------------*/
void SingularityLine::removeAllSingularityPoints()
{
  m_points.clear();
}
/*----------------------------------------------------------------------------*/
double SingularityLine::length() const
{
  double l=0;
...
/*----------------------------------------------------------------------------*/
void SurfaceSingularityLine::
setTraversedFaces(const std::vector<gmds::TCellID>& AIDs)
{
  m_traversed_faces_id = AIDs;
}
/*----------------------------------------------------------------------------*/
bool SurfaceSingularityLine::
getIntersectionPoint( SurfaceSingularityLine* ALine, 
		      ...
		      gmds::math::Point& APnt)
{
  ...
  for (unsigned int i = 1; i < m_discretization_points.size()-1; i++){
    gmds::math::Point pi = m_discretization_points[i];
    gmds::math::Point pj = m_discretization_points[i+1];
    if(ARefPnt.distance(pi)<=ARefRadius || ARefPnt.distance(pi)<=ARefRadius)
      segments_1.push_back(gmds::math::Segment(pi,pj));
  }

  for (unsigned int i = 1; i < other_pnts.size()-1; i++){
    gmds::math::Point pi = other_pnts[i];
    gmds::math::Point pj = other_pnts[i+1];
    if(ARefPnt.distance(pi)<=ARefRadius || ARefPnt.distance(pi)<=ARefRadius)
      segments_2.push_back(gmds::math::Segment(pi,pj));
  }

  bool found_intersection = false;

  for (unsigned int i = 0; 
       !found_intersection && i < segments_1.size(); i++) {

    gmds::math::Segment si = segments_1[i];

    for (unsigned int j = 0; 
       !found_intersection && j < segments_2.size(); j++) {
